...
| PyPI package | release target: [`silva-networks==1.2.2`](https://pypi.org/project/silva-networks/) |
...
| Docs source widget | top-right GitHub facts refresh against `source_release_tag: v1.2.2` before cached release data is reused |
...
| Full-scale family surface | all 64 canonical families have data, literature, benchmark, scale-control, and extension guides; compact dense/scalable equivalence checks are executable |
| Learned solver and backward policies | HyperDEQ, exact implicit gradients, JFB, SHINE, phantom gradients, and unrolling are documented with separate forward/backward controls and analytic checks |
| Circuit equilibrium family | QDEQ provides a differentiable compact statevector route, external measured-circuit adapter, image contracts, direct/implicit schedules, and Jacobian diagnostics |
| Guided navigation | all 210 navigable Markdown pages and 83 rendered notebooks include contextual next steps and an extension/reproduction path |
| Canonical notebook curriculum | all 109 package, bridge, and unreleased book/research notebooks include executable extension, equivalence, gradient, reproduction, and scaling material |
| Portable test suite | 385 core and extended tests pass with no skipped outcomes; device checks use CUDA when available and CPU otherwise |
...
| Notebook validation | `python scripts/run_notebook_smoke.py --all --timeout 180` | yes; all 109 canonical notebooks must execute |
...
| Zenodo archiving | Zenodo GitHub integration enabled for `jseluis/silva-networks`; v1.0.0, v1.1.0, v1.2.0, and v1.2.1 archived, and v1.2.2 metadata prepared | yes |
| Release trigger | push the validated annotated `v1.2.2` tag after `main` CI and Pages pass | yes |
...
- Sixty-four canonical family routes with explicit data contracts, primary sources, benchmark paths, scale controls, and extension points.
...
- Solver-learning, backward-approximation, quantum-equilibrium, source-equilibrium, evidence, and experiment-protocol APIs.
