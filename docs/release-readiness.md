...
| PyPI package | release target: [`silva-networks==1.2.1`](https://pypi.org/project/silva-networks/) |
...
| Docs source widget | top-right GitHub facts refresh against `source_release_tag: v1.2.1` before cached release data is reused |
...
| Full-scale family surface | all 44 canonical families have data, literature, benchmark, scale-control, and extension guides; compact dense/scalable equivalence checks are executable |
| Guided navigation | all 174 navigable Markdown pages and 56 rendered notebooks include contextual next steps and an extension/reproduction path |
| Canonical notebook curriculum | all 82 package, bridge, and unreleased book/research notebooks include executable extension, equivalence, gradient, reproduction, and scaling material |
| Portable test suite | 338 core and extended tests pass with no skipped outcomes; device checks use CUDA when available and CPU otherwise |
...
| Notebook validation | `python scripts/run_notebook_smoke.py notebooks/*.ipynb notebooks/package_api/*.ipynb notebooks/implicit_bridge/*.ipynb --timeout 180` | yes; all 82 canonical notebooks must execute |
...
| Zenodo archiving | Zenodo GitHub integration enabled for `jseluis/silva-networks`; v1.0.0, v1.1.0, and v1.2.0 archived, and v1.2.1 metadata prepared | yes |
| Release trigger | push the validated annotated `v1.2.1` tag after `main` CI and Pages pass | yes |
...
- Forty-four canonical family routes with explicit data contracts, primary sources, benchmark paths, scale controls, and extension points.
