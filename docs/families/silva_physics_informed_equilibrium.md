# `silva_physics_informed_equilibrium` Reproduction Dossier

**physics-informed ODE solution equilibrium.** This dossier connects the source mechanism to its SILVA
...
| Task contract | time collocation points plus dynamics/initial condition -> state trajectory |
| Source relation | `paper-adaptation` |
| References | [[51]](../paper/references.md#ref-51){ .silva-cite } |
| Repositories | <a href="https://github.com/brunompacheco/pideq" target="_blank" rel="noopener">https://github.com/brunompacheco/pideq</a> |
| Editable scale plan | `experiments/reproduction/configs/silva_physics_informed_equilibrium.json` |
...
- collocation batching
- matrix-free derivative solve
- Jacobian samples

## Constructor and Shape Contract

```python
silva_physics_informed_equilibrium(state_dim: 'int', output_dim: 'int', *, time_dim: 'int' = 1, hidden_dim: 'int | None' = None, state_scale: 'float' = 0.2, transition: 'nn.Module | None' = None, readout: 'nn.Module | None' = None, derivative_mode: 'DerivativeMode' = 'auto', dense_derivative_threshold: 'int' = 64, derivative_max_iter: 'int' = 50, derivative_tol: 'float' = 1e-06, config: 'SolverConfig | None' = None)
```
...
- Write and evaluate the family equation: `z_star(t)=T_theta(z_star(t),t); (I-dT/dz) dz_star/dt=dT/dt`
...
- record integral absolute error
- record ODE residual
- record initial-condition error
- record residual
...
- Van der Pol or declared nonlinear IVP
