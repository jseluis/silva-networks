# `silva_image_matting_equilibrium` Reproduction Dossier

**trimap-constrained image-matting equilibrium.** This dossier connects the source mechanism to its SILVA
...
| Task contract | B,3,H,W image and B,1,H,W trimap -> alpha matte |
| Source relation | `paper-adaptation` |
| References | [[111]](../paper/references.md#ref-111){ .silva-cite } |
| Repositories | <a href="https://github.com/CurioPocket/DEQ-Matt" target="_blank" rel="noopener">https://github.com/CurioPocket/DEQ-Matt</a> |
| Editable scale plan | `experiments/reproduction/configs/silva_image_matting_equilibrium.json` |
...
- crop resolution
- encoder width
- unknown-region fraction
- solver tolerance

## Constructor and Shape Contract

```python
silva_image_matting_equilibrium(image_channels: 'int' = 3, hidden_channels: 'int' = 16, *, contraction: 'float' = 0.5, encoder: 'nn.Module | None' = None, refiner: 'nn.Module | None' = None, config: 'SolverConfig | None' = None) -> 'None'
```
...
- Write and evaluate the family equation: `alpha_star=Project_trimap(sigmoid(R_theta(alpha_star,E_theta(image,trimap))))`
