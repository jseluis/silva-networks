# `multiscale_vision_deq` Reproduction Dossier

**simultaneous multiresolution vision equilibrium.** This dossier connects the source mechanism to its SILVA
...
| Task contract | B,C,H,W -> tuple of equilibrium feature maps |
| Source relation | `paper-adaptation` |
| References | [[5]](../paper/references.md#ref-5){ .silva-cite } |
| Repositories | <a href="https://github.com/locuslab/mdeq" target="_blank" rel="noopener">https://github.com/locuslab/mdeq</a> |
| Editable scale plan | `experiments/reproduction/configs/multiscale_vision_deq.json` |
...
```text
Z_star[r] = T_r(Z_star[1:R], X) for every resolution r
```
...
- attach detection or dense-prediction heads
- change cross-scale fusion
- resolution pyramid
- blocks per scale
- implicit backward
...
```python
multiscale_vision_deq(in_channels: 'int', channels: 'Sequence[int]', *, blocks_per_scale: 'int | Sequence[int]' = 1, expansion: 'float' = 5.0, big_kernel_counts: 'int | Sequence[int]' = 0, groups: 'int' = 4, dropout: 'float' = 0.0, stem_stride: 'int' = 1, injection_mode: 'MDEQInjectionMode' = 'highest', input_stem: 'nn.Module | None' = None, injection_modules: 'Sequence[nn.Module] | None' = None, transition_module: 'nn.Module | None' = None, config: 'SolverConfig | None' = None, **transition_kwargs)
```
...
- Write and evaluate the family equation: `Z_star[r] = T_r(Z_star[1:R], X) for every resolution r`
...
- record top-1 accuracy or mean IoU
