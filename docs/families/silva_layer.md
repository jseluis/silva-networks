# `silva_layer` Reproduction Dossier

**one structured equilibrium point.** This dossier connects the source mechanism to its SILVA
...
| Task contract | x, optional edge_index/batch -> state |
...
| Editable scale plan | `experiments/reproduction/configs/silva_layer.json` |
...
```text
z_star = sigma(S(x) + H(z_star) + L(z_star; E) + G(z_star; b))
```
...
- replace S, H, L, or G
- supply a custom shape-preserving operator
- state width
- operator sparsity
- solver history
...
```python
silva_layer(in_dim: 'int', hidden_dim: 'int', *, local: 'nn.Module | LocalOperatorName | None' = 'graph', global_term: 'nn.Module | GlobalOperatorName | None' = 'mean', self_term: 'nn.Module | SelfOperatorName | None' = None, config: 'SolverConfig | None' = None, activation: 'Callable[[Tensor], Tensor]' = <built-in method tanh of type object>, output_activation: 'Callable[[Tensor], Tensor]' = <built-in method tanh of type object>, normalize: 'bool' = True, local_kwargs: 'dict | None' = None, global_kwargs: 'dict | None' = None, self_kwargs: 'dict | None' = None) -> 'SILVALayer'
```
...
- Write and evaluate the family equation: `z_star = sigma(S(x) + H(z_star) + L(z_star; E) + G(z_star; b))`
...
| --- | ---: |
| Initial loss | `0.284432` |
| Final loss | `0.0240855` |
| Fractional loss reduction | `0.915` |
| Residual or final increment norm | `0.00551978` |
| Iterations or tied increments | `16` |
| Parameter count | `67` |
| Final gradient norm | `0.410538` |
