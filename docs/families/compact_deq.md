# `compact_deq` Reproduction Dossier

**affine-tanh DEQ in SILVA grammar.** This dossier connects the source mechanism to its SILVA
...
| Task contract | B,D input -> B,H equilibrium |
...
| Editable scale plan | `experiments/reproduction/configs/compact_deq.json` |
...
```text
z_star = tanh(W_z z_star + W_x x + b)
```
...
```python
compact_deq(in_dim: 'int', hidden_dim: 'int', *, config: 'SolverConfig | None' = None, normalize: 'bool' = False) -> 'SILVALayer'
```
...
- Write and evaluate the family equation: `z_star = tanh(W_z z_star + W_x x + b)`
...
- WikiText-103 or a declared sequence dataset
- analytic affine-tanh case
