# `fourier_operator_equilibrium` Reproduction Dossier

**Fourier field equilibrium.** This dossier connects the source mechanism to its SILVA
...
| Task contract | B,C,H,W forcing -> B,C_out,H,W |
...
| Repositories | <a href="https://github.com/neuraloperator/neuraloperator" target="_blank" rel="noopener">https://github.com/neuraloperator/neuraloperator</a> |
| Editable scale plan | `experiments/reproduction/configs/fourier_operator_equilibrium.json` |
...
```text
u_star = sigma(S(a,f) + F_inv(R_theta F(u_star)) + W u_star)
```
...
```python
fourier_operator_equilibrium(in_channels: 'int', state_channels: 'int', out_channels: 'int', *, modes_height: 'int' = 4, modes_width: 'int' = 4, field_scale: 'float' = 0.05, **kwargs)
```
...
- Write and evaluate the family equation: `u_star = sigma(S(a,f) + F_inv(R_theta F(u_star)) + W u_star)`
...
| --- | ---: |
| Initial loss | `0.474568` |
| Final loss | `0.224777` |
| Fractional loss reduction | `0.526` |
| Residual or final increment norm | `0.0407393` |
| Iterations or tied increments | `6` |
| Parameter count | `951` |
| Final gradient norm | `1.02719` |
