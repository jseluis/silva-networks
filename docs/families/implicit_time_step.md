# `implicit_time_step` Reproduction Dossier

**implicit ODE/PDE time step.** This dossier connects the source mechanism to its SILVA
...
| Task contract | state plus rhs and step size -> next state |
| Source relation | `paper-adaptation` |
| References | [[7]](../paper/references.md#ref-7){ .silva-cite } |
...
| Editable scale plan | `experiments/reproduction/configs/implicit_time_step.json` |
...
```text
u_next = u_now + dt F(u_next, context)
```
...
```python
implicit_time_step(rhs: 'nn.Module', step_size: 'float', *, config: 'SolverConfig | None' = None, projector: 'nn.Module | None' = None, transition_module: 'nn.Module | None' = None)
```
...
- Write and evaluate the family equation: `u_next = u_now + dt F(u_next, context)`
...
- record equation residual
- record stability error
