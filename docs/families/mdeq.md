# `mdeq` Reproduction Dossier

**compact coupled multiscale equilibrium.** This dossier connects the source mechanism to its SILVA
...
| Task contract | paired or derived resolutions -> coupled states |
...
| Editable scale plan | `experiments/reproduction/configs/mdeq.json` |
...
- scale widths
- cross-scale interpolation
- solver history

## Constructor and Shape Contract

```python
mdeq(in_dim: 'int', low_dim: 'int', high_dim: 'int', *, config: 'SolverConfig | None' = None, spectral_scale: 'float' = 0.45, reengage: 'bool' = True)
```
...
- CIFAR-10
- compact deterministic multiscale images
