# `silva_sparse_hyperspectral_equilibrium` Reproduction Dossier

**sparse-representation hyperspectral equilibrium.** This dossier connects the source mechanism to its SILVA
...
| Task contract | B,C,H,W noisy hyperspectral cube -> sparse code and reconstructed cube |
| Source relation | `paper-adaptation` |
| References | [[106]](../paper/references.md#ref-106){ .silva-cite } |
| Repositories | <a href="https://arxiv.org/abs/2203.15901" target="_blank" rel="noopener">https://arxiv.org/abs/2203.15901</a> |
| Editable scale plan | `experiments/reproduction/configs/silva_sparse_hyperspectral_equilibrium.json` |
...
```python
silva_sparse_hyperspectral_equilibrium(channels: 'int', code_channels: 'int', *, threshold: 'float' = 0.02, step_size: 'float' = 0.4, prior_scale: 'float' = 0.05, prior: 'nn.Module | None' = None, config: 'SolverConfig | None' = None) -> 'None'
```
...
- Write and evaluate the family equation: `c_star=soft_threshold(c_star-eta D_a(D_s c_star-y)+lambda D_a P_theta(D_s c_star))`
