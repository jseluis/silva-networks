# `silva_poisson_mirror_equilibrium` Reproduction Dossier

**positive Poisson inverse equilibrium.** This dossier connects the source mechanism to its SILVA
...
| Task contract | nonnegative observations plus forward/adjoint operators -> positive reconstruction |
| Source relation | `paper-adaptation` |
| References | [[50]](../paper/references.md#ref-50){ .silva-cite } |
| Repositories | <a href="https://github.com/christiandaniele/DEQ-MD" target="_blank" rel="noopener">https://github.com/christiandaniele/DEQ-MD</a> |
| Editable scale plan | `experiments/reproduction/configs/silva_poisson_mirror_equilibrium.json` |
...
```python
silva_poisson_mirror_equilibrium(*, transition: 'nn.Module | None' = None, initializer: 'nn.Module | None' = None, intensity_operator: 'Callable[[Tensor], Tensor] | None' = None, config: 'SolverConfig | None' = None)
```
...
- Write and evaluate the family equation: `u_star = mirror_Burg(u_star, grad Poisson(Au,y) + regularizer(u_star))`
...
- declared Poisson inverse-imaging data
