# `silva_projected_qp` Reproduction Dossier

**constrained projected optimization equilibrium.** This dossier connects the source mechanism to its SILVA
...
| Task contract | B,D parameters plus declared constraints -> feasible optimizer |
| Source relation | `paper-adaptation` |
| References | [[8]](../paper/references.md#ref-8){ .silva-cite }, [[9]](../paper/references.md#ref-9){ .silva-cite } |
| Repositories | <a href="https://github.com/cvxpy/cvxpylayers" target="_blank" rel="noopener">https://github.com/cvxpy/cvxpylayers</a><br><a href="https://github.com/jseluis/silva-networks" target="_blank" rel="noopener">https://github.com/jseluis/silva-networks</a> |
| Editable scale plan | `experiments/reproduction/configs/silva_projected_qp.json` |
...
```text
z_star = projection_C(z_star - alpha(Q z_star + c(x)))
```
...
```python
silva_projected_qp(in_dim: 'int', state_dim: 'int', *, constraint: 'ConstraintKind' = 'none', ridge: 'float' = 1.0, step_size: 'float' = 0.2, lower_bound: 'float | Tensor | None' = None, upper_bound: 'float | Tensor | None' = None, simplex_mass: 'float' = 1.0, equality_matrix: 'Tensor | None' = None, equality_rhs: 'Tensor | None' = None, projection_ridge: 'float' = 1e-08, config: 'SolverConfig | None' = None, reengage: 'bool' = True)
```
...
- Write and evaluate the family equation: `z_star = projection_C(z_star - alpha(Q z_star + c(x)))`
...
- analytic constrained quadratic programs
