# `silva_serialized_smoothing_equilibrium` Reproduction Dossier

**serialized randomized smoothing for equilibrium classifiers.** This dossier connects the source mechanism to its SILVA
...
| Task contract | B,D inputs plus Gaussian samples -> class counts and certified radii |
| Source relation | `paper-adaptation` |
| References | [[107]](../paper/references.md#ref-107){ .silva-cite } |
| Repositories | <a href="https://github.com/WeizhiGao/Serialized-Randomized-Smoothing" target="_blank" rel="noopener">https://github.com/WeizhiGao/Serialized-Randomized-Smoothing</a> |
| Editable scale plan | `experiments/reproduction/configs/silva_serialized_smoothing_equilibrium.json` |
...
```python
silva_serialized_smoothing_equilibrium(input_dim: 'int', state_dim: 'int', num_classes: 'int', *, sigma: 'float' = 0.25, contraction: 'float' = 0.5, config: 'SolverConfig | None' = None) -> 'None'
```
...
- Write and evaluate the family equation: `z_i_star=T_theta(z_i_star,x+sigma epsilon_i), z_i^(0)=stopgrad(z_(i-1)_star)`
