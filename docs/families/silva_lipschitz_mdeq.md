# `silva_lipschitz_mdeq` Reproduction Dossier

**Lipschitz-controlled simultaneous multiscale equilibrium.** This dossier connects the source mechanism to its SILVA
...
| Task contract | B,D input -> concatenated multiscale equilibrium -> task output |
| Source relation | `paper-adaptation` |
| References | [[99]](../paper/references.md#ref-99){ .silva-cite } |
| Repositories | <a href="https://github.com/iiduka-researches/Lipschitz_mdeq" target="_blank" rel="noopener">https://github.com/iiduka-researches/Lipschitz_mdeq</a> |
| Editable scale plan | `experiments/reproduction/configs/silva_lipschitz_mdeq.json` |
...
```text
z_star=tanh(S_theta(x)+W_hat z_star), ||W_hat||_infinity<=rho<1; z_star=concat(z_star[1],...,z_star[R])
```
...
```python
silva_lipschitz_mdeq(input_dim: 'int', scale_dims: 'Sequence[int]', output_dim: 'int', *, contraction: 'float' = 0.8, injection: 'nn.Module | None' = None, readout: 'nn.Module | None' = None, config: 'SolverConfig | None' = None) -> 'None'
```
...
- Write and evaluate the family equation: `z_star=tanh(S_theta(x)+W_hat z_star), ||W_hat||_infinity<=rho<1; z_star=concat(z_star[1],...,z_star[R])`
...
- notebooks/package_api/61_silva_lipschitz_mdeq.ipynb
