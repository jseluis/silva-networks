# `silva_joint_inference_equilibrium` Reproduction Dossier

**joint representation inference and input optimization.** This dossier connects the source mechanism to its SILVA
...
| Task contract | observations -> coupled representation and optimized input -> task output |
| Source relation | `paper-adaptation` |
| References | [[95]](../paper/references.md#ref-95){ .silva-cite } |
| Repositories | <a href="https://github.com/locuslab/JIIO-DEQ" target="_blank" rel="noopener">https://github.com/locuslab/JIIO-DEQ</a> |
| Editable scale plan | `experiments/reproduction/configs/silva_joint_inference_equilibrium.json` |
...
```text
(z_star,u_star)=(T_theta(z_star,u_star,y), Projection_C(u_star-eta g_phi(u_star,z_star,y)))
```
...
```python
silva_joint_inference_equilibrium(observation_dim: 'int', state_dim: 'int', optimized_input_dim: 'int', output_dim: 'int', *, representation_transition: 'nn.Module | None' = None, input_update: 'nn.Module | None' = None, readout: 'nn.Module | None' = None, config: 'SolverConfig | None' = None) -> 'None'
```
...
- Write and evaluate the family equation: `(z_star,u_star)=(T_theta(z_star,u_star,y), Projection_C(u_star-eta g_phi(u_star,z_star,y)))`
...
- record task metric
- record representation residual
- record optimized-input residual
