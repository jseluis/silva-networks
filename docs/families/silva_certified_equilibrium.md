# `silva_certified_equilibrium` Reproduction Dossier

**contractive equilibrium with interval and semialgebraic certificate routes.** This dossier connects the source mechanism to its SILVA
...
| Task contract | bounded B,D input -> bounded equilibrium and certified task output |
| Source relation | `paper-adaptation` |
| References | [[97]](../paper/references.md#ref-97){ .silva-cite }, [[98]](../paper/references.md#ref-98){ .silva-cite } |
| Repositories | <a href="https://openreview.net/forum?id=y1PXylgrXZ" target="_blank" rel="noopener">https://openreview.net/forum?id=y1PXylgrXZ</a><br><a href="https://github.com/locuslab/monotone_op_net" target="_blank" rel="noopener">https://github.com/locuslab/monotone_op_net</a> |
| Editable scale plan | `experiments/reproduction/configs/silva_certified_equilibrium.json` |
...
```text
z_star=phi(W z_star+U x+b), ||W||_infinity<1; [z_lower,z_upper]=IBP_fixed_point([x_lower,x_upper])
```
...
```python
silva_certified_equilibrium(input_dim: 'int', state_dim: 'int', output_dim: 'int', *, activation: "Literal['relu', 'tanh']" = 'relu', contraction: 'float' = 0.8, config: 'SolverConfig | None' = None) -> 'None'
```
...
- Write and evaluate the family equation: `z_star=phi(W z_star+U x+b), ||W||_infinity<1; [z_lower,z_upper]=IBP_fixed_point([x_lower,x_upper])`
...
- record natural accuracy
- record certified accuracy by radius
- record certificate margin
- record bound residual and iterations
...
- tests/test_advanced_expansions.py
