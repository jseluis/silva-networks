# `silva_bayesian_deq` Reproduction Dossier

**Bayesian equilibrium with sequential posterior inference.** This dossier connects the source mechanism to its SILVA
...
| Task contract | B,D input -> S posterior equilibria -> predictive mean and variance |
| Source relation | `paper-adaptation` |
| References | [[94]](../paper/references.md#ref-94){ .silva-cite } |
| Repositories | <a href="https://openreview.net/forum?id=hT9FJBePUR" target="_blank" rel="noopener">https://openreview.net/forum?id=hT9FJBePUR</a> |
| Editable scale plan | `experiments/reproduction/configs/silva_bayesian_deq.json` |
...
```python
silva_bayesian_deq(input_dim: 'int', state_dim: 'int', output_dim: 'int', *, transition: 'SILVABayesianTransitionProtocol | nn.Module | None' = None, readout: 'nn.Module | None' = None, posterior_samples: 'int' = 4, sequential: 'bool' = True, config: 'SolverConfig | None' = None) -> 'None'
```
...
- Write and evaluate the family equation: `theta_s~q_phi(theta); z_s_star=T_theta_s(z_s_star,x); p(y|x)=S^{-1} sum_s p(y|z_s_star)`
