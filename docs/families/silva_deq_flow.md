# `silva_deq_flow` Reproduction Dossier

**compact optical-flow equilibrium.** This dossier connects the source mechanism to its SILVA
...
| Task contract | B,C,H,W image pair -> B,2,H,W flow |
...
| Repositories | <a href="https://github.com/locuslab/deq-flow" target="_blank" rel="noopener">https://github.com/locuslab/deq-flow</a> |
| Editable scale plan | `experiments/reproduction/configs/silva_deq_flow.json` |
...
```text
flow_star = U_theta(flow_star, features, correlation)
```
...
- replace feature and update modules
- add sparse fixed-point correction
- feature stride
- correlation radius
- fixed-point reuse
...
```python
silva_deq_flow(in_channels: 'int' = 1, feature_dim: 'int' = 8, hidden_dim: 'int' = 32, corr_radius: 'int' = 1, update_scale: 'float' = 0.25, config: 'SolverConfig | None' = None, reengage: 'bool' = True, encoder_module: 'nn.Module | None' = None, update_block: 'nn.Module | None' = None, transition_module: 'nn.Module | None' = None)
```
...
- Write and evaluate the family equation: `flow_star = U_theta(flow_star, features, correlation)`
