# `message_passing_deq` Reproduction Dossier

**message-passing DEQ reduction.** This dossier connects the source mechanism to its SILVA
...
| Task contract | N,D features and sparse edge_index -> N,H state |
| Source relation | `paper-adaptation` |
| References | [[4]](../paper/references.md#ref-4){ .silva-cite }, [[16]](../paper/references.md#ref-16){ .silva-cite } |
| Repositories | <a href="https://github.com/locuslab/deq" target="_blank" rel="noopener">https://github.com/locuslab/deq</a> |
| Editable scale plan | `experiments/reproduction/configs/message_passing_deq.json` |
...
```python
message_passing_deq(in_dim: 'int', hidden_dim: 'int', *, config: 'SolverConfig | None' = None, local: 'nn.Module | LocalOperatorName | None' = 'graph', normalize: 'bool' = True, local_kwargs: 'dict | None' = None) -> 'SILVALayer'
```
...
- Write and evaluate the family equation: `Z_star = sigma(S(X) + L_G(Z_star))`
...
- citation or long-range graph data
- deterministic graph case
