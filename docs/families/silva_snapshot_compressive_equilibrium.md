# `silva_snapshot_compressive_equilibrium` Reproduction Dossier

**snapshot-compressive video reconstruction equilibrium.** This dossier connects the source mechanism to its SILVA
...
| Task contract | coded snapshot and frame masks -> reconstructed video volume |
| Source relation | `paper-adaptation` |
| References | [[104]](../paper/references.md#ref-104){ .silva-cite } |
| Repositories | <a href="https://github.com/IndigoPurple/DEQSCI" target="_blank" rel="noopener">https://github.com/IndigoPurple/DEQSCI</a> |
| Editable scale plan | `experiments/reproduction/configs/silva_snapshot_compressive_equilibrium.json` |
...
- spatial resolution
- mask ensemble
- volumetric prior width

## Constructor and Shape Contract

```python
silva_snapshot_compressive_equilibrium(frames: 'int', *, prior: 'nn.Module | None' = None, step_size: 'float' = 0.8, prior_scale: 'float' = 0.05, config: 'SolverConfig | None' = None) -> 'None'
```
...
- Write and evaluate the family equation: `v_star=D_theta(v_star+eta Phi^T(Phi Phi^T)^-1(y-Phi v_star))`
