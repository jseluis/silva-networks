# `silva_physics_graph_deq` Reproduction Dossier

**physics-guided graph equilibrium.** This dossier connects the source mechanism to its SILVA
...
| Task contract | node fields, edges, weights, velocities -> physical state/output |
| Source relation | `paper-adaptation` |
| References | [[44]](../paper/references.md#ref-44){ .silva-cite } |
...
| Editable scale plan | `experiments/reproduction/configs/silva_physics_graph_deq.json` |
...
```text
Z_star = sigma(S(X) + diffusion_G(Z_star) + advection_G(Z_star))
```
...
```python
silva_physics_graph_deq(in_dim: 'int', state_dim: 'int', out_dim: 'int', *, task: 'GraphTask' = 'node', pooling: 'GraphPooling' = 'mean', transition: 'nn.Module | None' = None, readout: 'nn.Module | None' = None, config: 'SolverConfig | None' = None)
```
...
- Write and evaluate the family equation: `Z_star = sigma(S(X) + diffusion_G(Z_star) + advection_G(Z_star))`
...
- air-quality sensor graph
- synthetic graph transport
...
- notebooks/package_api/18_silva_graph_transport_lab.ipynb
