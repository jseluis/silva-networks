# `silva_cortex` Reproduction Dossier

**one point with an arbitrary internal module graph.** This dossier connects the source mechanism to its SILVA
...
| Task contract | input tensor -> same-shape equilibrium state or custom readout |
...
| Editable scale plan | `experiments/reproduction/configs/silva_cortex.json` |
...
```text
z_star = sigma(E_x(x) + A_theta(z_star, x) + L(z_star) + G(z_star))
```
...
- checkpointed operator internals
- solver mode

## Constructor and Shape Contract

```python
silva_cortex(input_dim: 'int | None' = None, state_dim: 'int | None' = None, **kwargs) -> 'SILVACortexLayer'
```
...
- Write and evaluate the family equation: `z_star = sigma(E_x(x) + A_theta(z_star, x) + L(z_star) + G(z_star))`
